class Mailing extends React.Component {
  constructor(props) {
    ...
    this.state = {
      ...
      filterCheck: false,
      filterCompany:
        this.props.filterCompany &&
        this.props.filterCompany.replace(
          /[\f\n\r\t\v\u00A0\u1680\u180e\u2000\u2001\u2002\u2003\u2004\u2005\u2006\u2007\u2008\u2009\u200a\u2028\u2029\u2028\u2029\u202f\u205f\u3000'"]/g,
          ""
        ),
      filterOffice: null,
  ...
  sendDateToServer() {
    let letter = {
      recipients: this.state.elements
        .filter((elem) => elem.check === true)
        .map(function (elem) {
          return { id: elem.id, type_client: elem.type_client };
        }),
    ...
    if (this.state.view) {
      letter.resume = Object.values(this.state.resumes).filter(
  ...
  funDevelopFilter(name) {
    ...
    if (this.state.filterCheck) {
      contitions.push(name + ".check == " + this.state.filterCheck);
    }
    if (this.state.filterCompany) {
      contitions.push(
        name +
          '.company.toLowerCase().indexOf("' +
          this.state.filterCompany.toLowerCase() +
          '")>-1'
      );
    }
    if (this.state.filterOffice) {
      contitions.push(
        name +
          '.office.toLowerCase().indexOf("' +
          this.state.filterOffice.toLowerCase() +
          '")>-1'
      );
    }
    if (this.state.filterMain) {
      contitions.push(name + ".main == " + this.state.filterMain);
    }
    if (this.state.filterAgency) {
      contitions.push(name + ".recrutmentagency == " + this.state.filterAgency);
    }
    if (this.state.filterIndustry) {
      contitions.push(
        name +
          '.industry.toLowerCase().indexOf("' +
          this.state.filterIndustry.toLowerCase() +
          '")>-1'
      );
    }
    if (this.state.filterLocation) {
      contitions.push(
        name +
          '.location.toLowerCase().indexOf("' +
          this.state.filterLocation.toLowerCase() +
          '")>-1'
      );
    }
  ...
  onChangePrice(amount, view = false) {
    ...
    let priceOfEmail = view
      ? this.props.oneEmailForResume
      : this.props.oneEmailForAd;
    let minPrice = view
      ? this.props.minPriceForResume
      : this.props.minPriceForAd;
  ...
  viewElements() {
    ...
    let rez = this.state.elements.filter(
      function (elem) {
        return filters(elem);
      }.bind(this)
    );
    return rez.slice(
      this.state.page_size * this.state.page,
      this.state.page_size * (this.state.page + 1)
    );
  }
  ...
  render() {
    ...
    let stepOne = [
      <MailingListOfCompanies
        viewElements={this.viewElements}
      ...
      <Pagination
        ...
        onClickPage={this.handleDataUpdate}
      />,
    ];
    let stepTwo = (
      <StepTwo
      ...
      <div className="row hidden-sm hidden-xs">
        ...
        <div className="col-md-offset-6 col-lg-offset-6 col-xs-6 col-md-6">
          <button
            ...
            onClick={
              this.state.isStepTwo ? this.sendDateToServer : this.onClickNext
            }
          >
            {this.state.isStepTwo
              ? `Send and Pay ${price_formatted.to_format()}`
              : "Next"}
          </button>
        </div>
      </div>
    ...
    let size = this.state.isStepTwo ? null : (
      <div className="col-lg-5 col-md-5 col-sm-5 col-xs-12">
        <ChangeSize
          onChangeSize={this.handleDataUpdate}
          page_size={this.state.page_size}
          length={this.state.elements.length}
        />
      </div>
    );
    let tab = this.state.isStepTwo ? null : (
      <ul className="nav nav-tabs">
        <li>
          <a href={this.props.mailbox}>Mailbox</a>
        </li>
        <li className="active">
          <a>Mailing List</a>
        </li>
            ...
            >
              <div className="panel panel-default">
                <div className="panel-body">
                  ...
                  <div className="col-lg-5 col-md-5 col-sm-5 col-xs-12">
                    <strong>Price: </strong>
                    <span className="badge">{price_formatted.to_format()}</span>
                  </div>
                </div>
              </div>
            </div>
