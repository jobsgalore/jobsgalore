class SocialShareButton {
    static openUrl(url, width = 640, height = 480) {
        let left = (screen.width / 2) - (width / 2);
        let top = (screen.height * 0.3) - (height / 2);
    ...
    static share(el) {
        let get_tumblr_extra, hashtags, tumblr_params, via_str, whatsapp_app_url;
        if (el.getAttribute === null) {
            el = document.querySelector(el);
        }
        let site = el.getAttribute("data-site");
        let appkey = el.getAttribute("data-appkey") || '';
        let $parent = el.parentNode;
        let titleText =  el.getAttribute("data-" + site + "-title") || $parent.getAttribute('data-title') || '';

        let title = encodeURIComponent(titleText);
        let img = encodeURIComponent($parent.getAttribute("data-img") || '');
        let url = encodeURIComponent($parent.getAttribute("data-url") || '');
        let via = encodeURIComponent($parent.getAttribute("data-via") || '');
