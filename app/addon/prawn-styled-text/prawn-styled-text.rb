...
module PrawnStyledText
  ...
  DEF_MARGIN_UL = 15
  DEF_SYMBOL_UL = "\x95 "
  HEADINGS = { h1: 24, h2: 20, h3: 16, h4: 14, h5: 13, h6: 13, p:12 }
  ...
  def self.text_node( pdf, data )
    ...
    data.each do |part|
      ...
      when :h1
        ...
        context[:options][:'text-align'] = 'center'
      ...
      when :p
        context[:options][:size] = HEADINGS[tag]
        context[:options][:'margin-top'] = DEF_HEADING_T-8
        context[:options][:'line-height'] = DEF_HEADING_H-8
      when :i, :em # italic
        styles.push :italic
      when :li # list item
